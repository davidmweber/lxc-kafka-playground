...
curl -s -X POST -H "Content-Type: application/vnd.kafka.json.v2+json" \
  ...
  http://$address/topics/jsontest > /dev/null
...
echo -n "Consumer setup response: "
curl -s -X POST -H "Content-Type: application/vnd.kafka.v2+json" \
    --data '{"name": "my_consumer_instance", "format": "json", "auto.offset.reset": "earliest"}' \
    http://$address/consumers/my_json_consumer  | jq
echo
...
curl -s -X GET -H "Accept: application/vnd.kafka.json.v2+json" \
  http://$address/consumers/my_json_consumer/instances/my_consumer_instance/records | jq
echo
